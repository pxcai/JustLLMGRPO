            ...
            for batch_dict in self.train_dataloader:
                ...
                for key, values in reward_extra_infos_dict.items():
                    ...
                    if vals.size == 0 or not np.isfinite(vals).any():
                        continue
                    metrics[f"train-reward/{key}/mean"] = float(np.nanmean(vals))
                    metrics[f"train-reward/{key}/max"] = float(np.nanmax(vals))
                    metrics[f"train-reward/{key}/min"] = float(np.nanmin(vals))
