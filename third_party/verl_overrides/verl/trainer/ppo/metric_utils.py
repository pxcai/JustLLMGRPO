    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        ...
        for uid, var2vals in uid2var2vals.items():
            ...
            for var_name, var_vals in var2vals.items():
                ...
                # compute mean and std, ignoring NaNs for metrics that were not computed
                n_resps = len(var_vals)
                try:
                    var_vals_array = np.asarray(var_vals, dtype=float)
                except (TypeError, ValueError):
                    continue
                valid_mask = ~np.isnan(var_vals_array)
                valid_var_vals = var_vals_array[valid_mask]
                if valid_var_vals.size == 0:
                    continue
                metric = {f"mean@{n_resps}": float(np_mean(valid_var_vals))}

                n_valid_resps = int(valid_var_vals.size)
                if n_valid_resps > 1:
                    metric[f"std@{n_resps}"] = float(np_std(valid_var_vals))

                    # cache ns list
                    if n_valid_resps not in ns_cache:
                        ns_cache[n_valid_resps] = gen_ns(n_valid_resps)
                    ns = ns_cache[n_valid_resps]

                    # compute best/worst metrics
                    for n in ns:
                        # compute best/worst metrics
                        (bon_mean, bon_std), (won_mean, won_std) = bootstrap_metric(
                            data=valid_var_vals.tolist(),
                            subset_size=n,
                        ...
                        if has_pred:
                            # create vote_data
                            valid_pred_vals = np.asarray(pred_vals, dtype=object)[valid_mask]
                            vote_data = [
                                {"val": val, "pred": pred}
                                for val, pred in zip(valid_var_vals.tolist(), valid_pred_vals.tolist(), strict=True)
                            ]
                            # compute maj metrics
