...
class vLLMHttpServer:
    ...
    async def collective_rpc(
        ...
        if collective_rpc is None and hasattr(self.engine, "engine"):
            collective_rpc = self.engine.engine.collective_rpc
        if collective_rpc is None:
            raise AttributeError("vLLM engine does not expose collective_rpc")

        result = collective_rpc(
            method=method,
            timeout=timeout,
            args=args,
            kwargs=kwargs,
    ...
    async def run_server(self, args: argparse.Namespace):
        ...
        engine_cls = V1AsyncLLM if vllm_envs.VLLM_USE_V1 and V1AsyncLLM is not None else AsyncLLMEngine
        ...
        engine_client = engine_cls.from_vllm_config(
        ...
        if collective_rpc is None and hasattr(engine_client, "engine"):
            collective_rpc = engine_client.engine.collective_rpc
        if collective_rpc is not None:
            maybe_result = collective_rpc(
        ...
        if "vllm_config" in init_app_sig.parameters:
            ...
            model_config = (
                ...
                if hasattr(engine_client, "get_model_config")
    ...
    async def run_headless(self, args: argparse.Namespace):
        ...
        if run_headless is None:
            raise ImportError(
                "vLLM does not provide vllm.entrypoints.cli.serve.run_headless. "
                "This is only required for non-master nodes in multi-node vLLM rollout."
            )
    ...
    async def generate(
        ...
        if self.lora_as_adapter:
            ...
            if hasattr(self.engine, "list_loras"):
                ...
                loaded_loras = self.engine.engine.list_loras()
            else:
                loaded_loras = set()
    ...
    async def wait_for_requests_to_drain(self):
        ...
        # vLLM 0.8.x V0 AsyncLLMEngine does not expose wait_for_requests_to_drain.
        # Poll the underlying LLMEngine instead; during init this is normally a no-op.
        llm_engine = getattr(self.engine, "engine", None)
        if llm_engine is None or not hasattr(llm_engine, "has_unfinished_requests"):
            return
    ...
    async def _sleep_hybrid(self):
        ...
        if self.lora_as_adapter:
            ...
            sleep_level = 2
        await self.collective_rpc("sleep", kwargs={"level": sleep_level})
